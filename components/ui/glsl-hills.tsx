...
// Slightly brighter gray so the hills read against the dark bg
...
export function GLSLHills({
  ...
  useEffect(() => {
    ...
    // Use container dimensions, not window, for flexibility
    const getW = () => container.offsetWidth || window.innerWidth
    const getH = () => container.offsetHeight || window.innerHeight

    const renderer = new THREE.WebGLRenderer({ canvas, antialias: false, alpha: true })
    renderer.setClearColor(0x000000, 0)

    const scene = new THREE.Scene()
    const camera = new THREE.PerspectiveCamera(45, getW() / getH(), 1, 10000)
    camera.position.set(0, 16, cameraZ)
    camera.lookAt(new THREE.Vector3(0, 28, 0))

    const uniforms: { time: { type: string; value: number } } = {
      time: { type: 'f', value: 0 },
    }
